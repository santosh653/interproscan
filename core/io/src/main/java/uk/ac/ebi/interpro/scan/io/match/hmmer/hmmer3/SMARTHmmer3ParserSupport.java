...
import uk.ac.ebi.interpro.scan.model.raw.SMARTHmmer3RawMatch;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 * Support class to parse HMMER3 output into {@link SMARTHmmer3RawMatch}es.
 *
 * @author Phil Jones
 * @version $Id$
 */
public class SMARTHmmer3ParserSupport extends AbstractHmmer3ParserSupport<SMARTHmmer3RawMatch> {

    private static final Pattern MODEL_ACCESSION_LINE_PATTERN
            = Pattern.compile("^Accession:\\s+(SM[^\\s]+)\\s*$");
    ...
    @Override
    protected SMARTHmmer3RawMatch createMatch(final String signatureLibraryRelease,
                                                final HmmSearchRecord hmmSearchRecord,
                                                final SequenceMatch sequenceMatch,
                                                final DomainMatch domainMatch) {
        return new SMARTHmmer3RawMatch(
